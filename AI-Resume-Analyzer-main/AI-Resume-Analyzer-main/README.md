...
<details>
  <summary>Parser Stack</summary>
  <ul>
    <li><a href="https://github.com/OmkarPathak/pyresparser">pyresparser (optional)</a></li>
...
<summary>Storage</summary>
  <ul>
    <li>No database required for parser API</li>
  </ul>
</details>
...
To run the production parser service, perform the following tasks:
...
Configure env variables (optional but recommended):

```env
RESUME_PARSER_API_KEY=change-me
...
## Usage
- Your backend should call `POST /parse` with multipart field `file`.
- If `RESUME_PARSER_API_KEY` is set, send it as `X-API-Key`.
