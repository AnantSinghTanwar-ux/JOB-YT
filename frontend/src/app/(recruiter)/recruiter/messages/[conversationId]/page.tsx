...
const extractMessages = (payload: unknown): Message[] => {
  ...
  const wrapped = payload as { data?: unknown; messages?: unknown };
  if (Array.isArray(wrapped.messages)) return wrapped.messages as Message[];
...
const extractConversations = (payload: unknown): ApiConversation[] => {
  if (!payload || typeof payload !== 'object') return [];
  const wrapped = payload as { data?: unknown; conversations?: unknown };
...
export default function RecruiterMessageThreadPage({ params }: { params: Promise<{ conversationId: string }> }) {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      try {
        ...
        if (!mounted) return;
        setMessages(extractMessages(messageRes));

        const conversations = extractConversations(convoRes);
        const conversation = conversations.find((c) => c.id === conversationId);
        if (conversation) {
          const fallbackName = 'Candidate';
          setOtherUserName(conversation.participant?.name?.trim() || fallbackName);
  ...
  return (
    <div className="mx-auto flex h-[calc(100vh-120px)] w-full max-w-3xl flex-col overflow-hidden rounded-2xl border border-[#d7ddd0] bg-[#f8faf4]">
      <div className="flex items-center justify-between border-b border-[#d7ddd0] bg-white px-4 py-3">
        ...
        <button
          onClick={() => router.push(ROUTES.recruiterMessages)}
