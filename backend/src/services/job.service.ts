...
export const JobService = {
  ...
  async createJob(
    ...
    // Fire-and-forget: generate + store job description embedding for future ATS scoring.
    // Non-blocking — never delays the create response.
    if (job?.id && job?.description) {
      EmbeddingCacheService.getOrGenerate(job.description)
        .then((embedding) => {
          if (embedding) {
            return EmbeddingCacheService.storeJobEmbedding(job.id, embedding);
          }
        })
        .catch((err) =>
          console.warn(`${LOG_PREFIX} Failed to pre-generate job embedding for ${job.id}:`, err),
        );
    }
  ...
  async updateJob(jobId: string, recruiterId: string, data: Parameters<typeof JobModel.update>[1]) {
    ...
    const updated = await JobModel.update(jobId, filteredData);

    // Fire-and-forget: regenerate embedding if description changed.
    if (filteredData.description && updated?.id) {
      EmbeddingCacheService.getOrGenerate(filteredData.description as string)
        .then((embedding) => {
          if (embedding) {
            return EmbeddingCacheService.storeJobEmbedding(updated.id, embedding);
          }
        })
        .catch((err) =>
          console.warn(`${LOG_PREFIX} Failed to regenerate job embedding for ${updated.id}:`, err),
        );
    }
  ...
  async calculateMatchScore(userSkills: string[], jobSkills: string[]): Promise<{ matchScore: number; matchedSkills: string[]; missingSkills: string[] }> {
    ...
    const result = SkillMatchingService.calculateSkillMatch(userSkills, jobSkills);
    return {
  ...
  async analyzeSkillGap(userSkills: string[], requiredSkills: string[]): Promise<{ matchingSkills: string[], missingSkills: string[], suggestions: Record<string, string> }> {
      const { matchedSkills, missingSkills } = await JobService.calculateMatchScore(userSkills, requiredSkills);
      const suggestions: Record<string, string> = {};
      for (const skill of missingSkills) {
          suggestions[skill] = `Consider taking an online course or practicing projects related to ${skill}.`;
      }
      return { matchingSkills: matchedSkills, missingSkills, suggestions };
  },
  ...
  async generateInterviewQuestions(role: string, skills: string[]): Promise<{ questions: string[] }> {
      ...
      const prompt = await PromptTemplatesService.renderTemplate('interview_questions', {
        role,
        skills: skills.join(', '),
      });
      ...
      if (aiConfig.isConfigured) {
        try {
          const generated = await AIService.generateJSON<string[]>(prompt);
          if (Array.isArray(generated) && generated.every(item => typeof item === 'string')) {
            return { questions: generated.length > 0 ? generated : staticFallback };
          }
        } catch (error) {
          console.warn('[JobService] AI interview questions generation failed:', error);
        }
      }
  ...
  /**
   * Record a job view/impression asynchronously
   * Increments the views_count for analytics
   * Non-blocking operation to avoid impacting response time
   */
  async recordJobView(jobId: string): Promise<void> {
    try {
      await JobModel.incrementViews(jobId);
    } catch (err) {
      // Silently fail - view tracking should not break the request
      console.error(`Failed to record view for job ${jobId}:`, err);
    }
  },
};
