...
export const AuthService = {
  async register(email: string, password: string, role: UserRole, referralCode?: string) {
    ...
    if (!password || typeof password !== 'string') {
      throw new UnauthorizedError('Password is required', 'PASSWORD_REQUIRED');
    }
    ...
    if (existing) {
      ...
      await UserModel.updatePassword(existing.id, password_hash);

      return {
        id: existing.id,
        email: existing.email,
        role: existing.role,
        emailVerificationRequired: !existing.is_verified,
      };
    }

    const password_hash = await hashPassword(password);
    const verify_token = AUTO_VERIFY_EMAIL_ON_REGISTER ? null : Math.floor(100000 + Math.random() * 900000).toString();
    ...
    if (AUTO_VERIFY_EMAIL_ON_REGISTER) {
      ...
      // Fire webhook: user.registered (fire-and-forget)
      setImmediate(() => {
        WebhookService.fireEvent(WEBHOOK_EVENTS.USER_REGISTERED, {
          event: WEBHOOK_EVENTS.USER_REGISTERED,
          user_id: user.id,
          email: user.email,
          role: user.role,
          auth_provider: 'local',
          ...
          console.error('[AuthService] Webhook fire failed silently:', err);
        });
      });
    ...
    // This prevents EmailJS from making the signup request appear stuck.
    void sendEmail({
      to: email,
      subject: 'Your Verification Code - Hiring Platform',
      html: require('./otp.service').OtpService.otpEmailHtml(verify_token as string),
    }).catch((err) => {
  ...
  async login(email: string, password: string) {
    ...
    // OAuth-only accounts have no password hash. Guard against null/undefined
    // to prevent bcrypt.compare() from crashing with "data and hash arguments required".
    if (!user.password_hash) {
      logger.warn('Password login attempted on OAuth-only account', {
        userId: user.id,
        authProvider: (user as unknown as Record<string, unknown>).auth_provider ?? 'unknown',
      });
      throw new UnauthorizedError(
        'This account uses social login. Please sign in with your social provider (Google, GitHub, or LinkedIn).',
