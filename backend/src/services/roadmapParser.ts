...
import { RawRoadmapData, RawNode, slugify } from './roadmapLoader';
...
export function parseRoadmap(raw: RawRoadmapData): ParseResult {
  ...
  const visitedRawNodes = new Set<string>();
  let sortCounter = 0;
  ...
  const buildIdMapVisited = new Set<string>();
  for (let i = 0; i < raw.nodes.length; i++) {
    buildIdMap(raw.nodes[i], flatNodes, rawIdToSlug, i, buildIdMapVisited);
  }
...
function buildIdMap(
  ...
  if (node.children) {
    for (let i = 0; i < node.children.length; i++) {
      buildIdMap(node.children[i], flatNodes, idMap, i, visited);
    }
  }
}
