...
const GOOGLE_CLIENT = new OAuth2Client(); // Initialize without audience; audience is passed during verification
...
export const OAuthService = {
    ...
    try {
      ...
      if (
        ...
        try {
          ...
          if (parts.length !== 3) {
            throw Object.assign(new Error('Invalid JWT format'), { cause: error });
          }
  ...
  async verifyGithubToken(code: string): Promise<OAuthIdentity> {
    ...
    const clientId = ensureEnv(process.env.GITHUB_CLIENT_ID, 'GITHUB_CLIENT_ID_MISSING', 'GITHUB_CLIENT_ID');
  ...
  async verifyLinkedinToken(code: string, redirectUriParam?: string): Promise<OAuthIdentity> {
    if (!code) {
    ...
    const redirectUri = redirectUriParam || ensureEnv(
  ...
  async verify(provider: OAuthProvider, tokenOrCode: string, redirectUri?: string): Promise<OAuthIdentity> {
    if (!tokenOrCode) {
    ...
    switch (provider) {
      ...
      case 'linkedin':
        return this.verifyLinkedinToken(tokenOrCode, redirectUri);
      default:
        throw new UnauthorizedError('Unsupported OAuth provider', 'OAUTH_PROVIDER_UNSUPPORTED');
    }
  },

  async getGithubAccessTokenAndIdentity(code: string): Promise<{ accessToken: string; identity: OAuthIdentity }> {
    if (!code) {
      throw new UnauthorizedError('GitHub authorization code is required', 'GITHUB_CODE_REQUIRED');
    }
...
export const OAuthAuthService = {
  async login(provider: OAuthProvider, tokenOrCode: string, referralCode?: string, role?: 'applicant' | 'recruiter', redirectUri?: string) {
    logger.info(`[OAuthAuthService.login] Starting OAuth login`, { provider, role });
    const oauthUser = await OAuthService.verify(provider, tokenOrCode, redirectUri);
