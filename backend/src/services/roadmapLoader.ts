...
export async function loadRoadmap(slug: string): Promise<RawRoadmapData | null> {
  try {
    ...
    let source: 'local' | 'github' = 'github';

    // 1. Attempt to fetch `[slug].md` (contains frontmatter). This is now OPTIONAL.
    //    Some roadmaps only have a .json file and no .md file.
    try {
      const result = await getFileContent(slug, `${slug}.md`);
      mdContent = result.content;
      source = result.source;
    } catch {
      log('info', `No .md file found for ${slug}, will rely on .json only`);
    }

    const title = (mdContent ? extractFrontmatterField(mdContent, 'title') : null) || formatTitle(slug);
    const description = (mdContent ? extractFrontmatterField(mdContent, 'description') : null) || `${title} roadmap`;
    ...
    // 2. Try fetching `[slug].json` (the React Flow format roadmap.sh uses)
    ...
    try {
      ...
      log('info', `No JSON structure available for ${slug}: ${jsonErr instanceof Error ? jsonErr.message : String(jsonErr)}`);
    }

    // 3. Fallback to Markdown Parser (only if .md was available and JSON failed)
    if (!jsonLoaded) {
      if (!mdContent) {
        throw new Error(`No .md or .json file found for roadmap '${slug}'. Skipping.`);
      }
      log('info', `Falling back to markdown parser for ${slug}`);
...
function parseReactFlowJson(data: any, _slug: string) {
  const nodes: RawNode[] = [];
  const edges: RawEdge[] = [];
...
function parseMarkdownStructure(content: string, slug: string) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const title = match[2].replace(/\[(.*?)\]\(.*?\)/g, '$1').trim(); // Remove markdown links
