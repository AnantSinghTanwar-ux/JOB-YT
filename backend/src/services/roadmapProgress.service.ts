...
export const RoadmapProgressService = {
  ...
  async getUserRoadmapProgress(userId: string, roadmapId: string): Promise<RoadmapProgress> {
    // 1. Fetch roadmap (supports both UUID and slug)
    const isUuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(roadmapId);
    const roadmap = isUuid
      ? await RoadmapModel.findById(roadmapId)
      : await RoadmapModel.findBySlug(roadmapId);
    ...
    // 3. Fetch roadmap nodes using resolved UUID
    const nodes = await RoadmapNodeModel.findByRoadmap(roadmap.id);
    if (nodes.length === 0) {
      return {
  ...
  async recommendNextSkill(
    ...
    // 2. Get current progress using resolved UUID
    const progress = await this.getUserRoadmapProgress(userId, roadmap.id);

    // 3. If all nodes completed, nothing to recommend
    if (progress.pendingNodes.length === 0) {
      return null;
    }

    // 4. Fetch edges for DAG traversal using resolved UUID
    const edges = await RoadmapEdgeModel.findByRoadmap(roadmap.id);
    const nodes = await RoadmapNodeModel.findByRoadmap(roadmap.id);
