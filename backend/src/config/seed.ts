  ...
  try {
    ...
    await client.query(`
      CREATE TABLE IF NOT EXISTS "github_repos" (
        ...
        CONSTRAINT "github_repos_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "users"("id") ON DELETE CASCADE ON UPDATE NO ACTION
      );
    `);
    await client.query(`
      CREATE UNIQUE INDEX IF NOT EXISTS "github_repos_github_id_key" ON "github_repos"("github_id");
    `);
    await client.query(`
    ...
    await client.query(
      `
      INSERT INTO users (email, password_hash, role, is_verified, email_verified, referral_code, credit_balance)
      VALUES ($1, $2, 'admin', TRUE, TRUE, $3, 0)
    ...
    for (const r of recruiters) {
      const res = await client.query(
        `
        INSERT INTO users (email, password_hash, role, is_verified, email_verified, referral_code, credit_balance)
        VALUES ($1, $2, 'recruiter', TRUE, TRUE, $3, 50)
        ON CONFLICT (email) DO NOTHING
        RETURNING id
      `,
    ...
    for (const a of applicants) {
      const res = await client.query(
        `
        INSERT INTO users (email, password_hash, role, is_verified, email_verified, referral_code, credit_balance)
        VALUES ($1, $2, 'applicant', TRUE, TRUE, $3, 50)
        ON CONFLICT (email) DO NOTHING
        RETURNING id
      `,
    ...
    const jobs = [
      ...
      },
      {
        ...
        description: 'Google is looking for a Software Engineer Intern to join our Search team...',
      },
      {
        recruiter_idx: 1,
        ...
        salary_max: 65000,
        skills: ['Python', 'R', 'SQL'],
        description: 'Microsoft is looking for a Data Science Intern to analyze hybrid telemetry...',
      },
      {
        recruiter_idx: 2,
        ...
        description: 'Amazon is looking for a QA Intern to support automated release testing...',
      },
    ];
    ...
    const tiers = [
      { name: 'FREE', target_audience: 'STUDENT', price_monthly: 0, price_annual: 0, features: JSON.stringify({ maxJobs: 0, aiCreditsPerMonth: 10 }) },
      { name: 'PRO_STUDENT', target_audience: 'STUDENT', price_monthly: 9.99, price_annual: 99.99, features: JSON.stringify({ maxJobs: 0, aiCreditsPerMonth: 100 }) },
      { name: 'STARTER', target_audience: 'EMPLOYER', price_monthly: 49.00, price_annual: 490.00, features: JSON.stringify({ maxJobs: 3, aiCreditsPerMonth: 200 }) },
      { name: 'PRO', target_audience: 'EMPLOYER', price_monthly: 99.00, price_annual: 990.00, features: JSON.stringify({ maxJobs: 10, aiCreditsPerMonth: 500 }) },
      { name: 'ELITE', target_audience: 'EMPLOYER', price_monthly: 199.00, price_annual: 1990.00, features: JSON.stringify({ maxJobs: 30, aiCreditsPerMonth: 1500 }) },
      { name: 'ENTERPRISE', target_audience: 'EMPLOYER', price_monthly: 499.00, price_annual: 4990.00, features: JSON.stringify({ maxJobs: 999, aiCreditsPerMonth: 9999 }) },
    ];

    for (const tier of tiers) {
      await client.query(
        `
        INSERT INTO subscription_tiers (name, target_audience, price_monthly, price_annual, currency, features, is_active, created_at, updated_at)
        VALUES ($1, $2, $3, $4, 'USD', $5, TRUE, NOW(), NOW())
        ON CONFLICT (name) DO UPDATE 
        SET target_audience = EXCLUDED.target_audience, 
            price_monthly = EXCLUDED.price_monthly, 
            price_annual = EXCLUDED.price_annual, 
            features = EXCLUDED.features, 
            updated_at = NOW()
      `,
        [tier.name, tier.target_audience, tier.price_monthly, tier.price_annual, tier.features],
      );
      console.log(`  Subscription tier seeded: "${tier.name}"`);
    }

    console.log(`\nSeed complete. Summary:`);
    console.log(`  1 admin, 3 recruiters (500 credits each), 2 applicants, ${jobs.length} jobs, 9 applications, 6 subscription tiers`);
  } catch (err) {
