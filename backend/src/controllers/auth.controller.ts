...
export const AuthController = {
  ...
  async verifyEmail(req: Request, res: Response, next: NextFunction) {
    try {
      const data = await AuthService.verifyEmail(req.query.email as string, req.query.token as string);
  ...
  async githubLogin(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      // Sync GitHub repos after successful login
      await GithubService.syncUserRepos(data.user.id, data.tokens?.accessToken || '');
  ...
  async linkedinLogin(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const normalizedCode = code.trim();
      const normalizedState = state.trim();
      const _codeKey = `oauth_code:linkedin:${normalizedCode}`;
      ...
      const result = await OAuthAuthService.login('linkedin', normalizedCode, referralCode, role, redirectUri);
